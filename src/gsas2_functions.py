...
class Refiner:
    ...
    def save_refinement(self, save_gpx = True, save_ds = True):
        """
        ...
        Args:
            save_gpx (boolean): whether to save the gpx file in current working directory automatically. Defaults to True.
            save_ds (boolean): whether to save the ds file in current working directory automatically. Defaults to True.
        Returns:
    ...
    def plot_refinement_results(self, plt_range=None, clearplt=False):
        ...
        ax.set_title('Processed as:%s\n(R$_{wp}$=%.3f GoF=%.3f, Temp(C)=%s)'%(self.name.split('/')[-1],Rwp,GoF,self.ds.attrs['HAB_temperature']))
        #ax.set_title('NC from: %s\nProcessed as:%s\n(R$_{wp}$=%.3f GoF=%.3f, Temp(C)=%s)'%(self.ds.attrs['nc_from'].split('/')[-1],self.name.split('/')[-1],Rwp,GoF,self.ds.attrs['HAB_temperature']))
    ...
    def cleanup(self, mode='tree'):
        ...
        if mode=='tree' and os.path.isdir(self.gsas2_scratch):
            shutil.rmtree(self.gsas2_scratch)
        elif mode=='file':
...
class seqRefiner:
    ...
    def save_refinement(self, save_gpx = True, save_ds = True):
        ...
        Args:
            save_gpx (boolean): whether to save the gpx file in current working directory automatically. Defaults to True.
            save_ds (boolean): whether to save the ds files in current working directory automatically. Defaults to True.
        Returns:
            array: a tuple (length 2 array), with the gpx G2Project object and the ds_list array [self.gpx, self.ds_list].
        """
    ...
    def update_ds_file(self, index):
        """
        Updates the ds file at index in nc_path
        
        args:
            index (int): which particular ds file to save
        """
        self.ds_list[index].to_netcdf(self.nc_paths[index]+'.new.nc',engine="scipy")
        time.sleep(0.1)
        shutil.move(self.nc_paths[index]+'.new.nc',self.nc_paths[index])
    ...
    def update_gpx(self):
        ...
        Updates the gpx file in nc_path
        """
        shutil.copyfile(self.gpx['Controls']['data']['LastSavedAs'],'%s.gpx'%self.nc_paths[0][:-3])
    ...
    def plot_refinement_results_single(self, index, plt_range=None, clearplt=False):
        """
        ...
        #ax.set_title('NC from: %s\nProcessed as:%s\n(R$_{wp}$=%.3f GoF=%.3f, Temp(C)=%s)'%(self.ds_list[index].attrs['nc_from'].split('/')[-1],self.names[index].split('/')[-1],Rwp,GoF,self.ds_list[index].attrs['HAB_temperature']))
        ax.set_facecolor('#FFFED4')
        ...
        plt.show()
        plt.savefig('%s.png' % self.names[index][:-3])
    ...
    def plot_all(self, file_range = None, temp_read = True, plt_range=None, clearplt=False):
        ...
        def update(frame):
            ...
            if temp_read:
                try:
                    self.ds_list[frame].attrs['HAB_temperature']
                    #ax.set_title('NC from: %s\nProcessed as:%s\n(R$_{wp}$=%.3f GoF=%.3f, Temp(C)=%s)'%(self.ds_list[index].attrs['nc_from'].split('/')[-1],self.names[index].split('/')[-1],Rwp,GoF,self.ds_list[index].attrs['HAB_temperature']))
                except:
                    raise KeyError('No temperature data in file number:', frame)
                
                ax.set_title('Processed as:%s\n(R$_{wp}$=%.3f GoF=%.3f, Temp(C)=%s)'%(self.names[frame].split('/')[-1],Rwp,GoF,self.ds_list[frame].attrs['HAB_temperature']))
            else:
    ...
    def set_refine_background(self, num_coeffs):
        """
        Refines background. Increased coeffs generates closer fit.
    ...
    def set_refine_cell_params(self):
        """
        Refines cell parameters. This is done differently than for the individual refinement class, due to the GSASII sequential refinement approach.
    ...
    def set_refine_strain_broadening(self, type = 'isotropic'):
        """
        Sets strain broadening to be refined.
    ...
    def set_refine_size_broadening(self, type = 'isotropic'):
        """
        Sets size broadening to be refined.
    ...
    def set_refine_inst_parameters(self,inst_pars_to_refine=['X', 'Y', 'Z', 'Zero', 'SH/L', 'U', 'V', 'W']):
        """
        Sets a given list of instrument parameters to be refined. If no list is provided, uses defaults. Parameters must be valid in accordance with the GSASII package.
